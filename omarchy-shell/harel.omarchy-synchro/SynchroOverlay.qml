...
Item {
              ...
              Item {
                ...
                Column {
                  ...
                  Row {
                    ...
                    Repeater {
                      model: [
                        { label: "Branch", value: root.branch, icon: "󰘬" },
                        { label: "Changed files", value: String(root.dirty), icon: "󰜄" },
                        { label: "Ahead / behind", value: root.ahead + " / " + root.behind, icon: "󰓦" },
                        { label: "Remote", value: root.remoteState, icon: "󰒍" }
                      ]
                      BorderSurface {
                        required property var modelData
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (root.action !== "status" || root.actionFailed) root.detail = rendered
      root.message = root.actionFailed ? rendered.split("\n")[0] : "Action completed successfully."

      if (root.action === "status" && !root.actionFailed) {
        root.applyStatus(data)
        root.message = "Repository status refreshed."
